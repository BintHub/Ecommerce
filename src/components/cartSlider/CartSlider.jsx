...
const CartSlider = ({show,setShow}) => {
  ...
  return (
   ...
   <div onClick={handleClose} className={classNames('w-full h-full z-[100000000] bg-[#00000033] fixed top-0 left-0',{
     ...
     <div onClick={(e)=>e.stopPropagation()} className='bg-white text-black w-[300px] lg:w-[417px] min-h-[746px] fixed top-0 right-0 
            ...
            cart.map(each=>(
              <div key={each.id} className='flex flex-wrap flex-row gap-4 py-8 md:ml-6'>
               ...
               <div className='font-poppins font-normal text-base py-4'>
                {each.name} <br /><span className='font-poppins font-medium text-xs text-primary'>{each.price}</span>
                </div> 
    ...
    <div className='mt-10 py-4 ml-6'>
      <h1 className='font-poppins font-medium text-base'>Subtotal: Rp {String(total).includes('.')?String(total).padEnd(5,'0').concat('.000'):String(total).concat('.').padEnd(5,'0').concat('.000')}</h1>
    </div>
